...
trait ProjectPlugins {

  def useSnapshot: Boolean

  lazy val snapshot = "latest.integration"
  ...
  //lazy val multiJvm     = "com.typesafe.sbt"         % "sbt-multi-jvm"          % "0.3.11"
  //lazy val cpd          = "de.johoop" % "cpd4sbt" % "1.1.5"
  ...
  lazy val echo         = "com.typesafe.sbt"         %  "sbt-echo"              %  v("0.1.14")
  lazy val sbtCoreNext  = "org.scala-sbt"            %  "sbt-core-next"         %  "0.1.1"
}

object ProjectPlugins extends ProjectPlugins {
  override def useSnapshot = false
}